...
export default function ProfileMenu() {
  ...
  const linkItems = LINKS.map((link) => (
    ...
    >
      <span className={styles["pm-item-ico"]}>{link.icon}</span>
      <span className={styles["pm-item-label"]}>{link.label}</span>
    </motion.a>
  ));

  return (
    <div className={styles["profile-menu"]} ref={rootRef}>
      <motion.button
        type="button"
        className={styles["pm-badge"]}
      ...
      <AnimatePresence>
        {open ? (
          ...
          >
            <path
              className={styles["pm-arrow-line"]}
            ...
            />
            <path
              className={styles["pm-arrow-head"]}
        ...
        {open ? (
          <motion.div
            ...
            exit="exit"
          >
            <motion.div className={styles["pm-head"]} variants={itemVar}>
              <JBadge size={40} />
              <span className={styles["pm-id"]}>
                <span className={styles["pm-name"]}>Jayesh Jidge</span>
                <span className={styles["pm-role"]}>Software Engineer</span>
              </span>
            </motion.div>

            <div className={styles["pm-divider"]} aria-hidden="true" />

            <div className={styles["pm-links"]}>{linkItems}</div>
          </motion.div>
        ) : null}
      </AnimatePresence>
    </div>
