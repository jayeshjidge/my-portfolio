...
export default function ConnectCard() {
  const tiles = SOCIALS.map((s) => (
    <a
      key={s.name}
      className={`${styles["cc-tile"]} ${styles[`cc-tile--${s.tile}`]}`}
      href={s.href}
      target="_blank"
  ...
  return (
    <div className={styles["cc-card"]}>
      <span className={styles["cc-arrow"]} aria-hidden="true">
      ...
      <h3 className={styles["cc-title"]}>Connect with me</h3>
      <div className={styles["cc-tiles"]}>{tiles}</div>
    </div>
  );
}
