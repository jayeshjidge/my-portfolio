...
export default function Lanyard({
  ...
  return (
    <div className={styles["exp-lanyard-rig"]} ref={rigRef}>
      <ThatsMeNote />
      <motion.div
        className={`${styles["exp-lanyard-swing"]}${grabbing ? ` ${styles["is-grabbing"]}` : ""}`}
      ...
      >
        <motion.div
          ...
          aria-hidden="true"
        >
          <span className={styles["exp-strap-text"]}>
            {`${item.company} · ${item.company} · ${item.company} · `}
          </span>
        </motion.div>
        <MetalClasp id={item.badgeId} />
        <motion.div className={styles["exp-badge-3d"]} style={{ rotateY }}>
          <IdCard item={item} />
        </motion.div>
      </motion.div>
    </div>
