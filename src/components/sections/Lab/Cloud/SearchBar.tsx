...
export default function SearchBar({
  ...
  const resultEls = results.map((w, i) => (
    <li key={w.id} role="option" aria-selected={i === hi}>
      <button
        type="button"
        className={"lab-search-item" + (i === hi ? " is-hi" : "")}
        onMouseEnter={() => setHi(i)}
        onClick={() => choose(w)}
      >
        <span className="lab-search-item-ic" aria-hidden="true">
          <TechIcon id={w.id} size={16} color={w.focal ? FOCAL_INK : CAT_COLORS[w.cat].ink} />
        </span>
        <span className="lab-search-item-name">{w.label}</span>
        <span className="lab-search-item-cat">{deck.cats[w.cat].name}</span>
      </button>
    </li>
  ));

  return (
    <div className={"lab-search" + (open ? " is-open" : "")} ref={rootRef}>
      <div className="lab-search-field">
        <Search className="lab-search-icon" size={15} strokeWidth={2} aria-hidden="true" />
        <input
          ref={inputRef}
          type="text"
        ...
        {hasQuery ? (
          ...
          <kbd className="lab-search-kbd" aria-hidden="true">
            ⌘ K
          </kbd>
        )}
      </div>
      ...
      {open && (
        <div className="lab-search-drop">
          {!hasQuery ? (
            <p className="lab-search-hint">Start typing — try React, Next.js, GraphQL…</p>
          ) : results.length > 0 ? (
            <ul className="lab-search-list" id="lab-search-list" role="listbox">
              {resultEls}
            </ul>
          ) : (
            <div className="lab-search-empty">
