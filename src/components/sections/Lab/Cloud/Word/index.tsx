...
export default function Word({
  ...
  return (
    <button
      ...
      className={
        styles["lab-word"] +
        (word.focal ? ` ${styles["is-focal"]}` : "") +
        (pinned ? ` ${styles["is-pinned"]}` : "") +
        (highlight ? ` ${styles["is-hi"]}` : "") +
        (dim ? ` ${styles["is-dim"]}` : "")
      }
      style={style}
    ...
    >
      <span className={styles["lab-word-ic"]}>
        <TechIcon id={word.id} color={iconTint} />
      </span>
      <span className={styles["lab-word-lb"]}>{word.label}</span>
      {!word.focal && <span className={styles["lab-word-dot"]} aria-hidden="true" />}
    </button>
  );
}
