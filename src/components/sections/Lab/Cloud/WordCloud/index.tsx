...
export default function WordCloud({
      ...
      ? focusW.related
          ...
          .map((r) => {
            ...
            return (
              <path
                key={"r-" + r.id}
                className={`${styles["lab-spoke"]} ${styles["is-rel"]}`}
                d={curve(a.x, a.y, b.x, b.y)}
                stroke={focusColor}
                style={{ opacity: 0.65 }}
              />
  ...
  const onDouble = (e: React.MouseEvent) => {
    if ((e.target as HTMLElement).closest("[data-lab-word]")) return;
    onResetActive();
  };
  ...
  return (
    <div
      ref={vpRef}
      className={styles["lab-cloud-vp"]}
      onDoubleClick={onDouble}
      role="application"
      aria-label={`${deck.label} technology map. Hover a word to highlight it, click for details.`}
    >
      <div className={styles["lab-cloud-grid"]} aria-hidden="true" />

      <div key={deck.id} className={styles["lab-cloud-plane"]} style={{ transform: `scale(${zoom})` }}>
        <svg className={styles["lab-wires"]} viewBox="0 0 1000 640" preserveAspectRatio="none" aria-hidden="true">
          {hubEls}
          {relEls}
        </svg>
        {wordEls}
